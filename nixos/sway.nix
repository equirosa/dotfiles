...
let
  lockCommand =
    "${ pkgs.swaylock }/bin/swaylock --image ${ config.home-manager.users.kiri.xdg.cacheHome }/background_image -f";
  commonCommands = {
        ...
        sway = {
          ...
          config = {
            ...
            startup = [
              ...
              {
                command =
                  ''${ pkgs.swayidle }/bin/swayidle -w timeout 300 '${ lockCommand }' timeout 600 'swaymsg "output * dpms off"' resume 'swaymsg "output * dpms on"; before-sleep '${
                    lockCommand
                  }'';
              }
            ];
